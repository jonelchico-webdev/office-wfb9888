...
export default function SimpleModal({mutate, setMutate, open, setOpen, title, description, mutateQuery, name, mutateID}) {	

    const [remove] = useMutation(mutateQuery)
	
	async function mutateQueryBtn() {
        const res = await remove({variables:{id: mutateID, name: name, enabled: false}})

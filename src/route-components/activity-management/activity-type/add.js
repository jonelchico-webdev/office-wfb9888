...
import {ADD_ACTIVITY_TYPE_MUTATION} from '../../../queries-graphql/activity-management/event-list/mutation/add-activity-type'
...
export default function ActivityType(props) {
    ...
    const [activityTypeAdd] = useMutation(ADD_ACTIVITY_TYPE_MUTATION);
    ...
    async function mutateUpdate() {
        if (
            ...
            try {
                const res = await activityTypeAdd({
                    variables: {
                        name: state.name,
    ...
    return <Paper elevation={1}  >
        <Title pageTitle={strings.addNewActivity} />
        <Grid container direction="row" justify="space-between" alignItems="center">
            <Typography className={classes.paper} variant="h6">{strings.addNewActivity}</Typography>
        </Grid>
        <Divider light={true} />

...
import { MODIFY_COMMISSION, ADD_COMMISSION_RULE_DEFAULT, DELETE_COMMISSION_RULE } from '../../../queries-graphql/commission-system/mutation/commission-add-mutation'
...
export default function CommissionModify(props) {
    ...
    const [commissionRuleAddDefault] = useMutation(ADD_COMMISSION_RULE_DEFAULT)
    const [commissionRuleDelete] = useMutation(DELETE_COMMISSION_RULE)
    ...
    // commissionAgentsProfile = data.commission.affiliateProfiles.edges
    ...
    
    {
		if(commissionGameVendor.length === 0 ) {
			commissionGameTypeVendors.map((o, idx) => (
    ...
    async function addCommissionRule() {
        await commissionRuleAddDefault({
            variables: {
                commission: id
            }
        })

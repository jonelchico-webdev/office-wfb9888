...
export default function DividendStatistics(props) {
	...
	const { data, loading } = useCommissionPaymentsQuery({
		mutation: mutate,
		commissionableAction_CommissionType_In: "loss",
						...
						commissionPayments.map((o, idx) => {
							...
							return (
								<TableRow>
									...
									<TableCell>{o.node.statusChangedAt ? moment(o.node.statusChangedAt).format("YYYY-MM-DD HH:mm:ss") : "-"}</TableCell>
									<TableCell >
										<TextField
											InputProps={{

...
import  { COMMISSION_MANAGEMENT, DIVIDEND_MANAGEMENT_ADD, DIVIDEND_MANAGEMENT_MODIFY } from '../../paths';
...
export default function CommissionManagement(props) {
	...
	return <Paper elevation={1}>
			...
			<Grid container className={classes.paper} justify="space-between">
				...
				<Grid>
					<Button variant="contained" style={{backgroundColor: 'rgb(255, 0, 0)', color: '#fff'}} onClick={() => history.push(DIVIDEND_MANAGEMENT_ADD)}>{strings.newCommission}</Button>
				</Grid>
			</Grid>
			<Divider light={true}/>
						...
						commissions.map((o, index) =>  <TableRow key={index}>
							...
							<TableCell>
								<Grid container spacing={1} direction="row">
									<Grid item><Button size="small" onClick={() => history.push(`${DIVIDEND_MANAGEMENT_MODIFY}/${o.node.id}`)}><Typography color="primary">{strings.modify}</Typography></Button></Grid>

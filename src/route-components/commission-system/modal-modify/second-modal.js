...
export default function SecondModal({open, setOpen, commissionGameVendor, updateCommissionVendors}) {
    ...
    function commissionGameVendorHandler(event) {
        ...
        commissionGameVendor
        .find(o => { 
            return o.vendorID === id ? idVendor = o.id : null
        })
        
        if(!commissionGameVendor.find(o => o.vendorID === id)) {
            commissionGameVendor.push({vendorID: id, gameTypeID: name, enabled: checked, id: idVendor})
        } else {
            let index = commissionGameVendor.findIndex(o => o.vendorID === id) 
            if (index > -1) {
                commissionGameVendor[index]["enabled"] = checked 
                commissionGameVendor[index]["vendorID"] = id 
                commissionGameVendor[index]["gameTypeID"] = name 
                commissionGameVendor[index]["id"] = idVendor 
    ...
    return  <Dialog 
            ...
            >
                                ...
                                <Grid container style={{ maxWidth: 300 }} alignItems="center">
                                    <Grid item style={{ pmarginLeft: "1rem" }}>
                                        <Checkbox
                                            ...
                                            defaultChecked={Boolean(commissionGameVendor.find((o) => { return o.vendorID === vendor.node.id ? o.enabled : false }))}
                                            id={vendor.node.id}
                ...
                <Grid style={{paddingTop: 30, marginBottom: 30}} direction="column" alignItems="center" container>
                    <Grid item>
                        <Button style={{width: 110, marginRight:20, height: 30}} color="primary" variant="contained" onClick={updateCommissionVendors}>Save</Button>

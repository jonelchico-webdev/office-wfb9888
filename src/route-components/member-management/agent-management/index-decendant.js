...
import { useAgentManagementDecendantsQuery } from '../../../queries-graphql/member-management/agent-management'
...
function AgentManagementDecendantsQuery(rowsPerPage, before, after, values) {
    const { data, loading } = useAgentManagementDecendantsQuery({ rowsPerPage, before, after, values })
    if (loading) {
        return null
    }
...
export default function AgentManagementDecendants() {
    ...
    const affiliateProfilesDecendants = AgentManagementDecendantsQuery(rowsPerPage, before, after, values)
    ...
    return <Paper elevation={1}>
        ...
        <Grid container className={classes.padding} alignItems="center" spacing={4}>
            ...
            <Grid container className={classes.padding} direction="column" >
                {
                    affiliateProfilesDecendants !== null ?
                        <SimpleTable
                            tableProps={{ size: "small" }}
                            noBorder={true}
                            hasPagination={true}
                            pagination={pagination}
                            pageInfo={affiliateProfilesDecendants.descendants.pageInfo}
                            count={affiliateProfilesDecendants.descendants.totalCount}
                            columns={
                                <TableRow>
                            ...
                            rows={
                                affiliateProfilesDecendants.descendants === undefined || affiliateProfilesDecendants.descendants.edges.length === 0  ? 
                                <TableRow>
                                    <TableCell align="center" colSpan={12}>没有可用数据</TableCell>
                                </TableRow>
                                :
                                affiliateProfilesDecendants.descendants.edges.map((o, index) => {
                                    let lastLoginNoTime = moment(o.node.lastOnlineAt).format( 'YYYY-MM-DD')
                                    let selectedDate = moment(lastLoginNoTime)
                                    let today = moment(new Date())
                                    let noDaysNotLogin = o.node.lastOnlineAt ? today.diff(selectedDate, 'days') : null
                                    ...
                                    return <TableRow>
                                        ...
                                        <TableCell style={{whiteSpace: "nowrap"}} align="right">{!o.nodememberLevel ? 0 : o.nodememberLevel}</TableCell>
                                        ...
                                        <TableCell>{o.node.lastLoginIp} {o.node.notes}</TableCell>
                                        <TableCell>{o.node.lastOnlineAt ? moment(o.node.lastOnlineAt).format("YYYY-MM-DD HH:MM:SS") : null}</TableCell>
                                        <TableCell align="center">{noDaysNotLogin}</TableCell>
                            ...
                            //             <TableCell>{o.node.username}</TableCell>
                            //             <TableCell>{o.node.affiliateProfile.affiliateUrl}</TableCell>
                            //             <TableCell align="right">{!o.node.vipRating ? 0 : o.node.vipRating}</TableCell>
                            //             <TableCell align="right">{!o.nodememberLevel ? 0 : o.nodememberLevel}</TableCell>
                            //             <TableCell>{moment(o.node.registeredAt).format("YYYY-MM-DD HH:MM:SS")}</TableCell>
                            //             <TableCell>
                            //                 {o.node.isActive ? <Typography style={{ color: '#689f38' }}>{strings.enable}</Typography> : <Typography color="error">{strings.disable}</Typography>}
                            //             </TableCell>
                            //             <TableCell align="right">{o.node.balance}</TableCell>
                            //             <TableCell>{o.node.lastLoginIp} {o.node.notes}</TableCell>
                            //             <TableCell>{o.node.lastLogin ? moment(o.node.lastLogin).format("YYYY-MM-DD HH:MM:SS") : null}</TableCell>
                            //             <TableCell align="right">{noDaysNotLogin}</TableCell>

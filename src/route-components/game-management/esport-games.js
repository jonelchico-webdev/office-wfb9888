...
import { ESPORT_GAMES, ESPORT_GAME_MANAGEMENT } from '../../paths';
...
export default function ESportGames(props) {
	const classes = useStyles();
	const strings = useLanguages(ESPORT_GAMES);
	const { history } = props;
	...
	const { data, loading } = useGameManagementVendorQuery({ gameType: "R2FtZVR5cGVOb2RlOjcxMQ==", rowsPerPage, before, after, mutate, open })
	if (loading) {
		return <Loading />
	}
	...
	// 					<Grid item><Button size="small" variant="contained" color="primary" onClick={() => history.push(`${ESPORT_GAME_MANAGEMENT}/${vendorValue.gameVendor.id}`)}>{strings.gameManagement}</Button></Grid>
	...
	return <Paper elevation={1} className={classes.paper}>
		<Title pageTitle={strings.eSportGames} />
							...
							<TableRow>
								...
								<TableCell style={{ textAlign: "center" }}>
									<Grid container spacing={1} direction="row" justify="center">
										<Grid item><Button size="small" variant="contained" color="primary" onClick={() => history.push(`${ESPORT_GAME_MANAGEMENT}/${o.node.id}`)}>{strings.gameManagement}</Button></Grid>

...
import { ESPORT_GAME_MANAGEMENT } from '../../../paths';
...
export default function ESportGameManagement(props) {
	const classes = useStyles();
	const { history } = props;
	const strings = useLanguages(ESPORT_GAME_MANAGEMENT);
	...
	const { data, loading } = useGameManagementWithVendorQuery({ gameType: "R2FtZVR5cGVOb2RlOjcxMQ==", name: name , vendor: historyArr[4], rowsPerPage, before, mutated, after, page })
	...
	return <Paper elevation={1} className={classes.paper} >
		<Title pageTitle={strings.eSportGameManagement} />

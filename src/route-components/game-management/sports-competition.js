...
import  { SPORTS_COMPETITION } from '../../paths';
...
export default function SportsCompetition() {
	...
	const strings = useLanguages(SPORTS_COMPETITION);
	...
	const { data, loading } = useGameManagementVendorQuery({ gameType: "R2FtZVR5cGVOb2RlOjEwNTA=", rowsPerPage, before, after, mutate, open })
	if (loading) {
		return <Loading />
	}
	...
	return <Paper elevation={1} className={classes.paper}>
		<Title pageTitle={strings.sportsCompetition} />

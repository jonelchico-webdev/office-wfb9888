...
import {ContinueCancelModal, SortChangeModal} from '../../components';
import Title from '../../components/title';
import useLanguages from '../../hooks/use-languages';
import  { LOTTERY_GAME } from '../../paths';
...
export default function LotteryGame() {
	...
	const strings = useLanguages(LOTTERY_GAME);
	...
	const { data, loading } = useGameManagementVendorQuery({ gameType: "R2FtZVR5cGVOb2RlOjcxNA==", rowsPerPage, before, after, mutate, open })
	if (loading) {
		return <Loading />
	}
	...
	return <Paper elevation={1} className={classes.paper}>
		<Title pageTitle={strings.lotteryGame} />
